...
- Teste, ob ein anderer User, z.B. `korni` `crontab -l` oder `crontab -e` ausführen kann
- Falls ein Cronjob für `korni` existiert hat - wird dieser weiterhin ausgeführt oder nicht?
...
>[!NOTE]
> Das Paket `at` muss eventuell nachinstalliert werden mit `apt install at`.
